...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{llXX}
		\global\rownum=0\relax\textbf{Variable Name} & \textbf{Data Type} & \textbf{Use of the Variable}                                                                                                                      & \textbf{Validation Needed (If Applicable)}                                                                           \\ \midrule
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{lXlXX}
		\global\rownum=0\relax \textbf{Test} & \textbf{Test Data}                                                        & \textbf{Test Type} & \textbf{Test Reason}                                                                                                               & \textbf{Expected Outcome}                                      \\ \midrule
		\global\rownum=0\relax 1             & Ensure that the Pi receives power and can boot into the OS on the SD card & Normal             & The soution will be deployed on a raspberry Pi, the Pi therefore needs to be working                                               & The Pi should boot into the OS on the SD card                  \\
...
\begin{table}[H]
	...
	\rowcolors{1}{white}{tableShade}
	\begin{tabularx}{\textwidth}{lXlXX}
		\global\rownum=0\relax \textbf{Test} \verb|#| & \textbf{Test Data}            & \textbf{Test Type} & \textbf{Test Reason}           & \textbf{Expected Outcome}       \\ \midrule
		\global\rownum=0\relax 1   &  &  &  & \\
		2   &  &  &  & \\
		3   &  &  &  & \\
		4   &  &  &  & \\
		5   &  &  &  & \\
...
The \texttt{free -m} command gives us a rundown of memory usage on our system. If the total swapdisk size is what we set the new size as, then the swap-disk size change worked as needed in Criterion 4($ \alpha $)\\\\
Criterion 5($ \alpha $) governs what software we need on our system. Most importantly for the solution, we need a relatively new version of Python 3.x.x which is also compatible with OpenCV. The Python we have installed is 3.7.3 and we have version 4.1.0 of OpenCV, which is compatible.\\
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{lXXXl}
			\global\rownum=0\relax\textbf{Test} & \textbf{Test Data}                                                                               & \textbf{Expected Outcome}                                                 & \textbf{Actual Outcome}                                                                       & \textbf{Passed?} \\ \midrule
...
& The client connects via VNC and we are successful in having a desktop environment & YES \\
		4             & Running \texttt{Python} in the terminal to aquire the version number & The version matches a release within the last 6 months & The version was 3.7.4, which is new enough to be compatible with what we need & YES \\
...
<@\texttt{\textcolor{myred}{\# this is a basic runtime loop, i am to change this when i add the web interface to call this as a function}}@> 
	...
	cv2.imshow('camera', obj_videoFeed) 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=3in]{figs/camTestOutput}
	\caption{The window showing Haar detection working}
	\label{fig:camtestoutput}
\end{figure}
...
For the thickness of the bounding box, the last parameter (1) on \texttt{[25]} was changed to (2) to  have the box drawn as two pixels thick. As well as this, the \texttt{RGB} value was changed from \texttt{(0,0,0)} to \texttt{(0, 255, 0)} as this makes the box easier to see.
\begin{figure}[H]
	\centering
	\includegraphics[width=3in]{figs/camTestOutputAmended}
	\caption{The window showing the amended bounding box}
	\label{fig:camtestoutput}
\end{figure}
...
%https://realpython.com/python-refactoring/#:~:text=Remove%20ads-,Refactoring%20in%20Python,performance%2C%20or%20reduction%20in%20complexity.
